...
var FileUploader = function (params, twit) {
  ...
  if (self._file_stream) {
      assert(params.file_size, 'Must specify `file_size` when passing file stream.');
      assert(params.media_type, 'Must specify `media_type` when passing file stream.');
      assert(params.media_category, 'Must specify `media_category` when passing file stream.');
  }
  ...
  self._remoteUpload = !!self._file_stream;
...
FileUploader.prototype._initMedia = function (cb) {
  ...
  self._getFileInfoForUpload(function (err, mediaType, mediaFileSizeBytes) {
      var opts = {
          ...
          'shared': self._isSharedMedia
      };

      var media_category;
      if(!self._media_category) {
      ...
      opts['media_category'] = media_category;
